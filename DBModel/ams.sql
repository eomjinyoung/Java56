...
ALTER TABLE POST_ADDRS
  ADD CONSTRAINT PK_POST_ADDRS -- 우편주소정보 기본키
    PRIMARY KEY (
      POST_NO -- 우편번호
    );
...
ALTER TABLE MEMBERS
ADD COLUMN PHOTO_PATH VARCHAR(255) NULL AFTER VALID;
...
-- 수강생
ALTER TABLE STUDENTS
  ADD CONSTRAINT FK_POST_ADDRS_TO_STUDENTS -- 우편주소정보 -> 수강생
    FOREIGN KEY (
      POSTNO -- 우편번호
    )
    REFERENCES POST_ADDRS ( -- 우편주소정보
      POST_NO -- 우편번호
    );
